...
func TestConfigCallers(t *testing.T) {
	...
	testtools.EqString(t, "security.jwtSigningMethod", config.SecurityJWTSigningMethod(), "HS512")
	testtools.EqString(t, "security.jwtToken", config.SecurityJWTPassword(), "e86074797a09ccb62688c0fdf149ab18")
	testtools.EqString(t, "security.httpReferrerPolicy", config.SecurityHTTPReferrerPolicy(), "same-origin")
	testtools.EqString(t, "security.httpContentSecurityPolicy", config.SecurityHTTPContentSecurityPolicy(),
