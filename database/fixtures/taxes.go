...
func (f *Fixtures) taxes() error {
	...
	if _, err := f.inserterDatabase.Exec(taxGroupTranslationQuery, f.mainUserID, f.taxGroupID, f.englishLanguage.ID(), database.DBTranslationFieldName, "High"); err != nil {
		return errors.Trace(err)
	}
	if _, err := f.inserterDatabase.Exec(taxGroupTranslationQuery, f.mainUserID, f.taxGroupID, f.dutchLanguage.ID(), database.DBTranslationFieldName, "Hoog"); err != nil {
		return errors.Trace(err)
	}

	// Tax
	if _, err := f.inserterDatabase.Exec(taxQuery, f.mainUserID, f.taxGroupID, f.unitedKingdomCountry.ID(), 21.); err != nil {
		return errors.Trace(err)
	}
